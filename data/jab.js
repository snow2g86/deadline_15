const JAB = {
  "warrior": {
    "icon":"⚔️", "res":"fury", "maxRes": 10, "resRec": 0,
    "base": { "hp": 100, "atk": 25, "def": 10, "move": 3, "range": 1 },
    "growth": { "hp": [8,14], "atk": [2,4], "def": [1,2] },
    "actionRec": 1.0
  },
  "knight": {
    "icon":"🛡️", "res":"fury", "maxRes": 10, "resRec": 0,
    "base": { "hp": 150, "atk": 15, "def": 14, "move": 3, "range": 1 },
    "growth": { "hp": [12,18], "atk": [1,2], "def": [1,3] },
    "actionRec": 1.0
  },
  "assassin": {
    "icon":"🗡️", "res":"energy", "maxRes": 100, "resRec": 15,
    "base": { "hp": 70, "atk": 35, "def": 5, "move": 4, "range": 1 },
    "growth": { "hp": [5,9], "atk": [3,5], "def": [0,1] },
    "actionRec": 1.2
  },
  "mage": {
    "icon":"🔮", "res":"mana", "maxRes": 120, "resRec": 12,
    "base": { "hp": 65, "atk": 30, "def": 4, "move": 3, "range": 4 },
    "growth": { "hp": [5,8], "atk": [2,5], "def": [0,1] },
    "actionRec": 0.5
  },
  "archer": {
    "icon":"🏹", "res":"energy", "maxRes": 100, "resRec": 12,
    "base": { "hp": 60, "atk": 28, "def": 3, "move": 4, "range": 4 },
    "growth": { "hp": [5,9], "atk": [2,4], "def": [0,2] },
    "actionRec": 1.2
  },
  "priest": {
    "icon":"✝️", "res":"mana", "maxRes": 100, "resRec": 10,
    "base": { "hp": 70, "atk": 30, "def": 5, "move": 3, "range": 10 },
    "growth": { "hp": [6,10], "atk": [1,3], "def": [1,2] },
    "actionRec": 0.8
  },
  "novice": {
    "icon":"🔰", "res":"energy", "maxRes": 100, "resRec": 10,
    "base": { "hp": 85, "atk": 22, "def": 8, "move": 3, "range": 1 },
    "growth": { "hp": [7,12], "atk": [2,3], "def": [1,2] },
    "actionRec": 1.0
  },
  "summoner": {
    "icon":"📖", "res":"mana", "maxRes": 110, "resRec": 11,
    "base": { "hp": 60, "atk": 26, "def": 4, "move": 3, "range": 3 },
    "growth": { "hp": [4,8], "atk": [2,4], "def": [0,1] },
    "actionRec": 1.0
  },
  "shaman": {
    "icon":"🎭", "res":"mana", "maxRes": 100, "resRec": 10,
    "base": { "hp": 70, "atk": 28, "def": 6, "move": 3, "range": 3 },
    "growth": { "hp": [5,9], "atk": [2,4], "def": [1,2] },
    "actionRec": 1.0
  },
  "brawler": {
    "icon":"👊", "res":"energy", "maxRes": 100, "resRec": 12,
    "base": { "hp": 90, "atk": 32, "def": 7, "move": 3, "range": 1 },
    "growth": { "hp": [7,11], "atk": [3,5], "def": [0,2] },
    "actionRec": 1.0
  },
  "lancer": {
    "icon":"🔱", "res":"fury", "maxRes": 10, "resRec": 0,
    "base": { "hp": 95, "atk": 28, "def": 12, "move": 3, "range": 2 },
    "growth": { "hp": [8,13], "atk": [2,4], "def": [1,3] },
    "actionRec": 1.0
  },
  "sapper": {
    "icon":"💣", "res":"energy", "maxRes": 100, "resRec": 8,
    "base": { "hp": 75, "atk": 30, "def": 6, "move": 3, "range": 1 },
    "growth": { "hp": [5,9], "atk": [2,5], "def": [0,1] },
    "actionRec": 1.5
  },
  "summon_spirit": {
    "icon":"✨", "isSummon":true, "res":"none", "maxRes": 0, "resRec": 0,
    "base": { "hp": 15, "atk": 15, "def": 0, "move": 3, "range": 3 },
    "growth": { "hp": [0,0], "atk": [0,0], "def": [0,0] },
    "actionRec": 1.0
  },
  "summon_golem": {
    "icon":"🗿", "isSummon":true, "res":"none", "maxRes": 0, "resRec": 0,
    "base": { "hp": 30, "atk": 12, "def": 5, "move": 2, "range": 1 },
    "growth": { "hp": [0,0], "atk": [0,0], "def": [0,0] },
    "actionRec": 1.0
  }
};

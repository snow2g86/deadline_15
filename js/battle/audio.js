...
const Audio = {
  ...
  _subscribe() {
    EventBus.on('unit_attacked', ({ attacker }) => { this.sfxAtk(attacker.cls); });
    EventBus.on('unit_killed', () => { this.sfxKill(); });
    EventBus.on('unit_healed', () => { this.sfxHeal(); });
    EventBus.on('wave_spawn', () => { this.sfxWave(); });
    EventBus.on('battle_end', ({ win }) => {
  ...
  sfxCtx() {
    ...
    if (!this._sfxGainNode) {
      this._sfxGainNode = this._actx.createGain();
      this._sfxGainNode.gain.setValueAtTime(GameStore._sett.sfxVol, this._actx.currentTime);
      this._sfxGainNode.connect(this._actx.destination);
    }
    return this._actx;
  },
  sfxDest() { return this._sfxGainNode || this.sfxCtx().destination; },
  sfxGain(ctx, v, tt) { const g = ctx.createGain(); g.gain.setValueAtTime(v, tt || ctx.currentTime); return g; },

  sfxOsc(ctx, type, freq, start, dur, gainN, detune) {
    if (!GameStore._sett.sfxOn) return { o: null, g: null };
  ...
  sfxNoise(ctx, start, dur, gainN, filter) {
    if (!GameStore._sett.sfxOn) return;
    ...
    if (filter) {
      const f = ctx.createBiquadFilter(); f.type = filter.type || 'highpass'; f.frequency.setValueAtTime(filter.freq || 1000, start);
  ...
  bgmStart() {
    this.bgmStop();
    if (!GameStore._sett.bgmOn) return;
    if (this._bgmAudioPath) { this._bgmStartExternal(); return; }

    const ctx = this.sfxCtx();
    const vol = 0.16 * GameStore._sett.bgmVol;
    ...
    const noise = (time, dur, nvol, hpFreq) => {
      ...
      const g = ctx.createGain(); g.gain.setValueAtTime(nvol, time);
    ...
    const schedule = () => {
      ...
      while (startT + beatIdx * eighth < now + LOOK) {
        const tt = startT + beatIdx * eighth;
        ...
        if (b === 0) {
          note('sine', bassN[ci] / 2, tt, eighth * 7.8, 0.08, 0);
          note('sine', bassN[ci] / 2 * 1.005, tt, eighth * 7.8, 0.05, 3);
        }
        const bF = bassN[ci];
        if (b % 2 === 0) {
          note('sawtooth', bF, tt, eighth * 1.6, 0.28, -3);
          note('sine', bF / 2, tt, eighth * 1.6, 0.12, 0);
        } else if (b === 3 || b === 7) {
          note('sawtooth', bF * 1.5, tt, eighth * 0.6, 0.14, -3);
        }
        if (b === 0 || b === 4) {
          const ko = ctx.createOscillator(), kg = ctx.createGain();
          ko.type = 'sine'; ko.frequency.setValueAtTime(95, tt);
          ko.frequency.exponentialRampToValueAtTime(20, tt + 0.2);
          kg.gain.setValueAtTime(b === 0 ? 0.50 : 0.35, tt); kg.gain.exponentialRampToValueAtTime(0.001, tt + 0.2);
          ko.connect(kg); kg.connect(comp); ko.start(tt); ko.stop(tt + 0.21);
          noise(tt, 0.04, 0.16, 1200);
        }
        if (b === 3 && beat32 >= 16) {
          const ko = ctx.createOscillator(), kg = ctx.createGain();
          ko.type = 'sine'; ko.frequency.setValueAtTime(85, tt);
          ko.frequency.exponentialRampToValueAtTime(25, tt + 0.12);
          kg.gain.setValueAtTime(0.18, tt); kg.gain.exponentialRampToValueAtTime(0.001, tt + 0.12);
          ko.connect(kg); kg.connect(comp); ko.start(tt); ko.stop(tt + 0.13);
        }
        if (b === 2 || b === 6) {
          noise(tt, 0.12, 0.28, 2200);
          note('triangle', 180, tt, 0.06, 0.10, 0);
          noise(tt + 0.04, 0.08, 0.10, 1600);
        }
        if ((b === 5 || b === 7) && beat16 >= 8) { noise(tt, 0.06, 0.10, 2800); }
        if (b % 2 === 0) { noise(tt, 0.03, beat16 % 4 === 0 ? 0.10 : 0.06, 10000); }
        if (beat32 % 16 === 14 && b === 6) { noise(tt, 0.14, 0.13, 6500); }
        if (beat32 % 16 < 8) {
          const melodyFreqs = [82.4, 98, 110, 123.47, 82.4, 110, 98, 123.47];
          const mf = melodyFreqs[(beatIdx / 2) % 8];
          if (b === 0 || b === 4) {
            note('triangle', mf * 2, tt + eighth * 0.5, eighth * 0.8, 0.07, 0);
            note('sine', mf * 2.05, tt + eighth * 0.5, eighth * 0.8, 0.03, 5);
          }
        }
        beatIdx++;
      }
  ...
  _bgmStartExternal() {
    ...
    fetch(this._bgmAudioPath)
      ...
      .then(buf => ctx.decodeAudioData(buf, (decoded) => {
        ...
        src.buffer = decoded;
        const vol = GameStore._sett.bgmVol * 0.85;
        const gain = ctx.createGain();
        gain.gain.setValueAtTime(vol, ctx.currentTime);
        src.connect(gain); gain.connect(ctx.destination);
        src.loop = true; src.start(ctx.currentTime);
  ...
  bgmStop() {
    if (this._bgmAudioSource) {
      ...
      this._bgmAudioSource = null; return;
    }
    if (!this._bgm) return;
    if (this._bgm.timer) { clearTimeout(this._bgm.timer); this._bgm.timer = null; }
    const m = this._bgm.master, ctx = this.sfxCtx(), tt = ctx.currentTime;
    m.gain.linearRampToValueAtTime(0, tt + 0.6);
    setTimeout(() => { try { m.disconnect(); } catch (e) {} }, 700);
    this._bgm = null;
  },

  setBattleMusic(audioPath) { this._bgmAudioPath = audioPath; },

  // ── Audio Settings ──
  setBGMOn(v) {
    GameStore._sett.bgmOn = v;
    if (v) { if (!this._bgm && !FSM.is(BattleState.BATTLE_END)) this.bgmStart(); }
    else { this.bgmStop(); }
    Renderer.saveSett();
  },
  setSFXOn(v) {
    GameStore._sett.sfxOn = v;
    if (this._sfxGainNode) this._sfxGainNode.gain.setValueAtTime(v ? GameStore._sett.sfxVol : 0, this.sfxCtx().currentTime);
    Renderer.saveSett();
  },
  setBGMVol(v) {
    GameStore._sett.bgmVol = v / 100;
    document.getElementById('sp-bgm-val').textContent = v;
    if (this._bgm && this._bgm.master) this._bgm.master.gain.setValueAtTime(0.12 * (v / 100), this.sfxCtx().currentTime);
    Renderer.saveSett();
  },
  setSFXVol(v) {
    GameStore._sett.sfxVol = v / 100;
    document.getElementById('sp-sfx-val').textContent = v;
    if (this._sfxGainNode && GameStore._sett.sfxOn) this._sfxGainNode.gain.setValueAtTime(v / 100, this.sfxCtx().currentTime);
    Renderer.saveSett();
  },
};

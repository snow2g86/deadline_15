...
Object.assign(G,{
  ...
  _initStageSelectPage(){
    ...
    availableStages.forEach((st)=>{
      ...
      b.innerHTML=`<div class="sb-num">${st.id}</div><div class="sb-name">${t('stages.stage_'+st.id+'_name')}</div><div class="sb-info">${t('stages.enemy_count', {count: st.tot})}</div><div class="sb-stars">${cl?'⭐':'☆'}</div>`;
      b.onclick=()=>{
        this._loadParty();
        // 죽은 유닛 제거

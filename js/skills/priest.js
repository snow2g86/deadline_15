...
registerSkill('priest_massheal', {
	...
	exec(u, tx, ty, sk, G) {
		...
		targets.forEach(tgt => {
			...
			G.vfxSpawn(G.uSX(tgt.x,tgt.y)+UCX, G.uSY(tgt.x,tgt.y)+UCY, {count:8,colors:['#4f4','#8f8','#fff'],shape:'ring',speed:2,spread:8,decay:0.025,size:6});
		});
		G.sfxHeal(); G.floatT(u.x, u.y, t('messages.priest_mass_heal'), 'heal');
		G.vfxSpawn(G.uSX(u.x,u.y)+UCX, G.uSY(u.x,u.y)+UCY, {count:15,colors:['#4f4','#ff8','#fff'],shape:'ring',speed:3,spread:14,decay:0.02,size:8});
		G._grantExp(u, 'heal');
		_skillDone(u, G);
	}
});
...
registerSkill('priest_purify', {
	...
	exec(u, tx, ty, sk, G) {
		...
		G.vfxSpawn(G.uSX(tgt.x,tgt.y)+UCX, G.uSY(tgt.x,tgt.y)+UCY, {count:15,colors:['#88f','#aaf','#fff'],shape:'ring',speed:3,spread:14,decay:0.02,size:6});
		G._grantExp(u, 'heal');
		_skillDone(u, G);
	}
});
...
registerSkill('priest_sanctuary', {
	...
	exec(u, tx, ty, sk, G) {
		...
		G.vfxSpawn(G.uSX(u.x,u.y)+UCX, G.uSY(u.x,u.y)+UCY, {count:20,colors:['#fbbf24','#ff8','#fff'],shape:'ring',speed:4,spread:18,decay:0.02,size:8});
		G._grantExp(u, 'heal');
		_skillDone(u, G);
	}
});
